...
def help_message():
   print("Usage: [Input_Image] [Output_Directory]")
   print("[Input_Image]")
...
def draw_line(event,x,y,flags,param):
    global old_x, old_y, drawing, StartSegmentation, BGMarked, FGMarked
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        if drawing == True:
            # Write code here to draw a line as and how the mouse pointer moves.
            cv2.line(img_marking, (old_x, old_y), (x, y), line_colour, 5)
            old_x = x
            old_y = y
        ...
        # Check if both fg and bg areas are marked. If yes then start segmentation.
        if StartSegmentation == False:
            # Check if both fg and bg areas are marked. If yes then start segmentation.
            if BGMarked == False and np.where((img_marking == [255, 0, 0]).all(axis = 2))[0].size > 0:
                BGMarked = True

            if FGMarked == False and np.where((img_marking == [0, 0, 255]).all(axis = 2))[0].size > 0:
                FGMarked = True

            if BGMarked == True and FGMarked == True:
                StartSegmentation = True

        if StartSegmentation == True:
            # Create Segmentation mask and output.

            # print("User has selected segmentation. Showing img_marking...")
            # img_marking[np.where((img_marking == [0, 0, 0]).all(axis = 2))] = [255, 255, 255]
...
if __name__ == '__main__':
    ...
    # drawing = True if mouse left button is pressed down, False if not pressed.
    # line_colour = (255, 0, 0) i.e. Blue if drawing line for Background. (0, 0, 255) i.e. Red if drawing line for Foreground.
    # old_x, old_y - store the old (x, y) positions of the mouse after clicking so that freehand drawing can be done.
    # StartSegmentation - "True" = can start segmentation as bg and fg are marked. "False" = bg and fg aren't marked yet.
    drawing = False
    line_colour = (255, 0, 0)
    old_x, old_y = -1, -1
    ...
    # Create a mask initially filled with 255, on which the user marks the foreground and background.
    img_marking = np.ones_like(img) * 255

    cv2.namedWindow("InputImage")
    cv2.namedWindow("Output")
    cv2.setMouseCallback("InputImage", draw_line)

    while(1):

        # Calculate SuperimposedImage
        SuperimposedImage = img.copy()
        SuperimposedImage[np.where((img_marking == [255, 0, 0]).all(axis = 2))] = [255, 0, 0]
        SuperimposedImage[np.where((img_marking == [0, 0, 255]).all(axis = 2))] = [0, 0, 255]
        cv2.imshow("InputImage", SuperimposedImage)

        # Wait for input from the user
        k = cv2.waitKey(1) & 0xFF
        if k == ord("b"):
            # Background marking i.e. Blue line.
            line_colour = (255, 0, 0)

        elif k == ord("f"):
            # Foreground marking i.e. Red line.
            line_colour = (0, 0, 255)
